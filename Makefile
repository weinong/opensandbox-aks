...
local-config:
	@set -euo pipefail; \
		...
		migrate_default '^OPEN_SANDBOX_CONTROLLER_VERSION[[:space:]]*\?=[[:space:]]*0\.1\.0$$' 'OPEN_SANDBOX_CONTROLLER_VERSION ?= 0.2.0'; \
		...
		add_if_missing OPEN_SANDBOX_CONTROLLER_VERSION 'OPEN_SANDBOX_CONTROLLER_VERSION ?= 0.2.0'; \
		add_if_missing OPEN_SANDBOX_CLI_VERSION 'OPEN_SANDBOX_CLI_VERSION ?= 0.1.1'; \
		add_if_missing ENABLE_SNAPSHOT_REGISTRY_SECRET 'ENABLE_SNAPSHOT_REGISTRY_SECRET ?= false'; \
		add_if_missing OPEN_SANDBOX_SNAPSHOT_REGISTRY 'OPEN_SANDBOX_SNAPSHOT_REGISTRY ?= $$(ACR_NAME).azurecr.io/opensandbox-snapshots'; \
		add_if_missing OPEN_SANDBOX_SNAPSHOT_REGISTRY_INSECURE 'OPEN_SANDBOX_SNAPSHOT_REGISTRY_INSECURE ?= false'; \
		add_if_missing OPEN_SANDBOX_SNAPSHOT_SECRET 'OPEN_SANDBOX_SNAPSHOT_SECRET ?= opensandbox-snapshot-registry'; \
		add_if_missing OPEN_SANDBOX_IMAGE_COMMITTER_IMAGE 'OPEN_SANDBOX_IMAGE_COMMITTER_IMAGE ?= opensandbox/image-committer:v0.1.0'; \
